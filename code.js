...
module.exports = ${JSON.stringify(config, null, 2)}`;
        ...
        return __async(this, null, function* () {
          ...
          for (const def of atomVariableDefinitions) {
            ...
            const floatFallbacksDesktop = {
              ...
              "Badge/font-size": 14,
              "Badge/icon-size": 16,
              "Badge/line-height": 16,
              "Badge/gap": 4,
              "Badge/radius": 999,
              "Badge/font-weight": 500
            };
            const floatFallbacksTablet = {
              "Button/padding-y": 10,
              "Button/padding-x": 20,
              "Button/font-size": 14,
              "Button/icon-size": 20,
              ...
              "Badge/font-size": 12,
              "Badge/icon-size": 14,
              "Badge/line-height": 14,
              "Badge/gap": 3,
              "Badge/radius": 999,
              "Badge/font-weight": 500
            };
            const floatFallbacksMobile = {
              "Button/padding-y": 8,
              "Button/padding-x": 16,
              "Button/font-size": 12,
              "Button/icon-size": 16,
        ...
        return __async(this, null, function* () {
          try {
            ...
            if (config.output === "page") {
              ...
              yield figma.setCurrentPageAsync(container);
            } else {
            ...
            if (config.components.buttons) {
              ...
              const buttonStyles = styles && styles.length > 0 ? styles : ["filled"];
              ...
              for (const variant of variants) {
                for (const style of buttonStyles) {
                  for (const state of states) {
                    ...
                    const btn = yield createButton(variant, state.toLowerCase(), style, config, findVar, atomVars);
      ...
      function createButton(variant, state, style, config, findVar, atomVars) {
        return __async(this, null, function* () {
          ...
          if (variant === "primary") {
            ...
            if (isOutlined) {
              if (state === "hover")
                ...
                bgVarTerms = ["action/secondaryactive", "secondaryactive"];
              else
                bgVarTerms = [];
            ...
            if (isOutlined) {
              if (state === "hover")
                ...
                bgVarTerms = ["action/destructiveactive", "destructiveactive"];
              else
                bgVarTerms = [];
      ...
      function createSemanticTokens(config) {
        return __async(this, null, function* () {
          try {
            ...
            yield updateProgress("Creating font-family aliases...");
            const fontFamilyMap = [
            ...
            for (const item of fontFamilyMap) {
              ...
              if (!aliasVar) {
                ...
                const primitiveVar = allVars.find(
                  (v) => v.variableCollectionId === sourceCollectionId && (v.name === primitiveName || v.name === item.name.replace("Typography/", ""))
                );
                if (primitiveVar) {
            ...
            yield updateProgress("Creating font-weight aliases...");
            const fontWeightMap = [
        ...
        return __async(this, null, function* () {
          ...
          try {
            ...
            const TOKEN_SCHEMA = [
              ...
              { name: "Action/destructiveText", light: "50", dark: "950", useStatus: "error" },
              // Text on destructive button
              // Status
